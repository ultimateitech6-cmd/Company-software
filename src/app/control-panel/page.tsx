...
export default function AdminControlPanel() {
  ...
  // Local Storage Login check + session expiry
  useEffect(() => {
    ...
    if (loginTime) {
      ...
      // Token exists but no loginTime (legacy session) — backfill loginTime from now
  ...
  // Session expiry interval — checks every 60 seconds
  useEffect(() => {
    if (!isAuthenticated) return;
    const interval = setInterval(() => {
      const loginTime = localStorage.getItem("adminLoginTime");
      if (!loginTime) { handleLogout(); return; }
      const elapsed = Date.now() - parseInt(loginTime, 10);
      if (elapsed >= SESSION_DURATION_MS) {
  ...
  if (isCheckingAuth) {
    return (
      <div className={`min-h-screen flex items-center justify-center p-4 relative font-sans transition-colors duration-300 ${adminTheme === "dark" ? "bg-slate-950" : "bg-slate-50"
        }`}>
  ...
  if (!isAuthenticated) {
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center p-4 relative overflow-hidden font-sans transition-colors duration-300 ${isDark ? "bg-slate-950" : "bg-slate-50"
        }`}>
        ...
        <div className="absolute top-6 left-6 z-30">
          <button
            ...
            type="button"
            className={`p-2.5 rounded-xl border transition-all cursor-pointer hover:scale-105 shadow-md flex items-center justify-center ${isDark
        ...
        <div className={`absolute top-1/4 left-1/4 w-[35rem] h-[35rem] rounded-full blur-[120px] -z-10 transition-colors duration-300 ${isDark ? "bg-indigo-900/10" : "bg-indigo-500/5"
          }`} />
        <div className={`absolute bottom-1/4 right-1/4 w-[30rem] h-[30rem] rounded-full blur-[100px] -z-10 transition-colors duration-300 ${isDark ? "bg-emerald-950/10" : "bg-emerald-500/5"
          }`} />

        <div className={`w-full max-w-md border rounded-3xl p-8 relative transition-all duration-300 ${isDark
          ...
          <div className="absolute top-0 right-0 p-6">
            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase border transition-colors ${isDark
          ...
          <div className="text-center space-y-3 mb-8 mt-4">
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mx-auto shadow-inner border transition-colors ${isDark
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-1.5 text-left">
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/80 border transition-all duration-200 ${isDark
            ...
            <div className="space-y-1.5 text-left">
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••••••••"
                  className={`w-full pl-4 pr-10 py-2.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/80 border transition-all duration-200 ${isDark
                      ? "bg-slate-950 border-slate-800 text-white placeholder-slate-700"
                      : "bg-slate-50 border-slate-200 text-slate-900 placeholder-slate-450"
                    }`}
                />
                <button
                  ...
                  className={`absolute right-3.5 top-1/2 -translate-y-1/2 cursor-pointer transition-colors ${isDark ? "text-slate-450 hover:text-white" : "text-slate-400 hover:text-slate-700"
                    }`}
                >
            ...
            {sessionExpired && (
              <div className={`flex items-center gap-2 p-3 rounded-xl text-xs border ${isDark
                ? "bg-amber-500/10 border-amber-500/20 text-amber-400"
                : "bg-amber-50 border-amber-200 text-amber-700"
              }`}>
                <Clock className="w-4 h-4 flex-shrink-0" />
                <span>Your session expired after 1 hour. Please sign in again.</span>
              </div>
            )}
            ...
            <button
              type="submit"
              className={`w-full py-2.5 px-4 bg-gradient-to-r from-indigo-600 to-indigo-850 hover:from-indigo-550 hover:to-indigo-800 text-white rounded-xl font-bold text-sm transition-all hover:-translate-y-0.5 cursor-pointer mt-2 ${isDark ? "shadow-lg shadow-indigo-950/50" : "shadow-md shadow-indigo-500/10"
                }`}
            >
  ...
  return (
    <div className={`h-screen overflow-hidden flex flex-col md:flex-row font-sans relative transition-colors duration-300 ${adminTheme === "dark"
        ? "bg-slate-950 text-slate-100"
        : "bg-slate-50 text-slate-900"
      }`}>
      {/* Dynamic Saving Indicator Toast */}
      {isSaving && (
        <div className={`fixed bottom-6 right-6 border p-4 rounded-xl shadow-2xl flex items-center gap-3 z-50 animate-pulse ${adminTheme === "dark" ? "bg-slate-900 border-slate-800 text-slate-300" : "bg-white border-slate-200 text-slate-700"
          }`}>
      ...
      {successMessage && (
        <div className={`fixed bottom-6 right-6 border p-4 rounded-xl shadow-2xl flex items-center gap-3 z-50 ${adminTheme === "dark" ? "bg-slate-900 border-emerald-500/25 text-slate-350" : "bg-white border-emerald-500/30 text-slate-700"
          }`}>
      ...
      {/* Sidebar Navigation */}
      <aside className={`w-full md:w-64 h-full overflow-y-auto flex flex-col justify-between flex-shrink-0 z-20 border-b md:border-b-0 md:border-r transition-colors duration-300 ${adminTheme === "dark"
              ...
              return (
                ...
                >
                  ...
                  {tab.badge !== undefined && tab.badge > 0 && (
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-black ${isActive ? "bg-white text-indigo-950" : "bg-slate-800 text-slate-300"
                      }`}>
                      {tab.badge}
                    </span>
                  )}
        ...
        <div className={`p-6 border-t ${adminTheme === "dark" ? "border-slate-850" : "border-slate-200"}`}>
          ...
          <div className={`flex items-center gap-1.5 px-4 py-2 mb-2 rounded-lg text-[10px] font-semibold ${
            adminTheme === "dark" ? "text-slate-500 bg-slate-900/60" : "text-slate-400 bg-slate-100/80"
          }`}>
            <Clock className="w-3 h-3 flex-shrink-0" />
            <span>Session expires in 1 hr from login</span>
          </div>
          <button
            onClick={handleLogout}
            className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-xs font-bold rounded-xl transition-colors cursor-pointer ${adminTheme === "dark"
                ? "text-slate-300 hover:text-rose-400 hover:bg-rose-500/10 bg-slate-900/40"
                : "text-slate-700 hover:text-rose-600 hover:bg-rose-50 bg-slate-100"
              }`}
          >
      ...
      <main className="flex-1 h-full flex flex-col overflow-hidden w-full z-10 max-w-7xl mx-auto px-6 py-4 md:px-8 md:py-5">
        ...
        <header className={`flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b pb-3 mb-4 flex-shrink-0 transition-colors duration-300 ${adminTheme === "dark" ? "border-slate-800/80" : "border-slate-200"
          }`}>
          <div>
            <h2 className={`text-xl md:text-2xl font-black tracking-tight capitalize ${adminTheme === "dark" ? "text-white" : "text-slate-900"
              }`}>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              onClick={toggleAdminTheme}
              className={`p-2 rounded-xl border transition-colors cursor-pointer ${adminTheme === "dark"
            ...
            <button
              onClick={() => fetchData()}
              disabled={loading}
              className={`px-3 py-1.5 border rounded-xl text-xs font-bold transition-colors cursor-pointer flex items-center gap-1.5 disabled:opacity-50 ${adminTheme === "dark"
                ...
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                      ...
                      return (
                        <div key={idx} className={`border rounded-2xl p-5 shadow-sm space-y-2 ${adminTheme === "dark" ? "bg-slate-900/40 border-slate-850/80" : "bg-white border-slate-200"
                          }`}>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className={`lg:col-span-2 border rounded-2xl p-5 shadow-sm space-y-3 ${adminTheme === "dark" ? "bg-slate-900/40 border-slate-850/80" : "bg-white border-slate-200"
                      }`}>
                      <h3 className={`text-xs font-bold uppercase tracking-widest flex items-center gap-1.5 ${adminTheme === "dark" ? "text-slate-400" : "text-slate-650"
                        }`}>
                      ...
                      <div className="h-44 w-full flex items-end justify-between pt-4 px-2">
                        {[15, 30, 24, 45, 55, 38, 62].map((height, i) => (
                          <div key={i} className="flex flex-col items-center gap-2.5 w-[10%]">
                            <div className={`w-full rounded-lg overflow-hidden h-32 flex items-end ${adminTheme === "dark" ? "bg-slate-850" : "bg-slate-100"
                              }`}>
                              <div
                    ...
                    <div className={`lg:col-span-1 border rounded-2xl p-5 shadow-sm space-y-4 ${adminTheme === "dark" ? "bg-slate-900/40 border-slate-850/80" : "bg-white border-slate-200"
                      }`}>
                    ...
                    <div className={`lg:col-span-3 border rounded-2xl p-5 shadow-sm space-y-4 ${adminTheme === "dark" ? "bg-slate-900/40 border-slate-850/80" : "bg-white border-slate-200"
                      }`}>
                      <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                        Latest Inquiries
                      </h3>
                      {leads.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          {leads.slice(0, 3).map((lead) => (
                            <div key={lead.id} className={`p-3 rounded-xl border border-l-2 border-l-indigo-500 flex justify-between items-start ${adminTheme === "dark" ? "bg-slate-955/60 border-slate-850" : "bg-slate-50 border-slate-200"
                              }`}>
                              <div>
                                <h4 className={`text-xs font-bold ${adminTheme === "dark" ? "text-white" : "text-slate-900"}`}>{lead.name}</h4>
                                <p className={`text-[10px] mt-1 truncate max-w-[150px] ${adminTheme === "dark" ? "text-slate-450" : "text-slate-500"
                                  }`}>{lead.email}</p>
                              </div>
                              <span className={`text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-wider ${lead.status === "New"
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative max-w-xs">
                    ...
                    <input
                      ...
                      className={`w-full pl-9 pr-4 py-1.5 text-xs rounded-xl border focus:outline-none focus:ring-1 focus:ring-indigo-500 ${adminTheme === "dark"
                  ...
                  <div className={`border rounded-2xl overflow-hidden shadow-sm transition-colors ${adminTheme === "dark" ? "bg-slate-900/40 border-slate-850/80" : "bg-white border-slate-200"
                    }`}>
                    {leads.length === 0 ? (
                        ...
                        return (
                          <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse text-xs">
                              <thead>
                                <tr className={`border-b font-bold uppercase tracking-wider text-[10px] ${adminTheme === "dark" ? "border-slate-800 bg-slate-850/40 text-slate-450" : "border-slate-200 bg-slate-100 text-slate-600"
                                  }`}>
                              ...
                              <tbody className={`divide-y ${adminTheme === "dark" ? "divide-slate-850" : "divide-slate-200"}`}>
                                {filtered.map((lead) => (
                                  <tr key={lead.id} className={`transition-colors ${adminTheme === "dark" ? "hover:bg-slate-900/30" : "hover:bg-slate-50"
                                    }`}>
                                    <td className="p-4">
                                    ...
                                    <td className="p-4">
                                      <select
                                        ...
                                        className={`px-2.5 py-1 rounded font-bold text-[9px] border focus:outline-none cursor-pointer tracking-wider uppercase ${lead.status === "New"
                                            ? "bg-indigo-500/10 border-indigo-500/20 text-indigo-400"
                                            : lead.status === "Contacted"
                                              ? "bg-amber-500/10 border-amber-500/20 text-amber-400"
                                    ...
                                    <td className="p-4 text-right space-x-2">
                                      <button
                                        ...
                                        className={`p-1.5 rounded transition-colors cursor-pointer inline-flex items-center ${adminTheme === "dark" ? "bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white" : "bg-slate-100 hover:bg-slate-200 text-slate-650 hover:text-slate-955"
                                          }`}
              ...
              {activeTab === "pricing" && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  {dbData.pricingPlans.map((plan) => (
                    <div key={plan.id} className={`border rounded-3xl p-6 flex flex-col justify-between shadow-sm relative transition-colors ${plan.isPopular
                      ...
                      <div className={`mt-8 pt-4 border-t ${adminTheme === "dark" ? "border-slate-850" : "border-slate-200"}`}>
                        <span className={`w-full text-center py-2.5 rounded-xl border text-xs font-bold block ${adminTheme === "dark"
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {filtered.map((post) => (
                          <div key={post.slug} className={`border rounded-2xl overflow-hidden shadow-sm flex flex-col justify-between group transition-colors duration-300 ${adminTheme === "dark" ? "bg-slate-900/40 border-slate-855/80" : "bg-white border-slate-200"
                            }`}>
                            <div className={`aspect-video relative overflow-hidden ${adminTheme === "dark" ? "bg-slate-955" : "bg-slate-100"
                              }`}>
                              {/* eslint-disable-next-line @next/next/no-img-element */}
                              <img
                                src={post.image}
                              ...
                              <span className={`absolute top-3 right-3 text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-wider ${post.isPublished !== false
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {dbData.testimonials.map((test) => (
                      <div key={test.id} className={`border rounded-2xl p-4.5 flex flex-col justify-between shadow-sm transition-colors duration-300 ${adminTheme === "dark" ? "bg-slate-900/40 border-slate-850/80" : "bg-white border-slate-200"
                        }`}>
                        <div className="space-y-3">
                          ...
                          <p className={`text-[11.5px] leading-relaxed font-medium italic ${adminTheme === "dark" ? "text-slate-350" : "text-slate-700"
                            }`}>
                            "{test.content}"
                          </p>
                        </div>

                        <div className={`pt-4 mt-5 border-t flex items-center justify-between ${adminTheme === "dark" ? "border-slate-850" : "border-slate-200"
                          }`}>
